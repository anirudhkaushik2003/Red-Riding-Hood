...
# load images
bg_img = pygame.image.load("img/formbg1.png").convert_alpha()
    ...
    def update(self, game_over):
        ...
        if game_over == 0:
           ...
           # if self.walk_music > walk_music_cooldown and self.in_air == False:
            ...
            #update scroll
            if (self.rect.right > (screen_width - scroll_thresh) and self.direction == 1) or (self.rect.left < scroll_thresh and self.direction == -1):
            ...
            for tile in row:
                ...
                if tile == 11:
                    ...
                    img = pygame.transform.scale(
                        tree_img, (tree_width, tree_height)
                    )
                    img_rect = img.get_rect()
...
screen_scroll = 0
y_scroll = 0
dummy_arrow2 = Arrow_powerup1_1(3,175)
...
while run:
    ...
    if player.powerup_index != 0 and set_iterations == False:
        powerup_iterations = 1000
        set_iterations = True
        dummy_arrow2 = Arrow_powerup1_1(25,150)
        powerup_group.add(dummy_arrow2)
        

    if set_iterations == True:
        
        pygame.draw.rect(
            screen, (0, 255, 255), (55, 140, (powerup_iterations / 10), 15)
        )

    if powerup_iterations == 0:
        if set_iterations == True:
            plant_fx.play()
            powerup_group.remove(dummy_arrow2)
        set_iterations = False
